...
const fetchData = async (pageNumber = 1, search = "") => {
  ...
  try {
    ...
    const response = await Api.get(
      `/api/banks?page=${pageNumber}&search=${search}`
    );
    ...
    await Swal.fire({
      ...
      text: error.response?.data?.meta?.message || "Gagal mengambil data bank. Silakan coba lagi.",
    ...
    <div class="mx-auto max-w-7xl p-4 sm:p-6 lg:p-8">
      <div class="rounded-3xl bg-white/80 backdrop-blur-xl shadow-2xl border border-indigo-100 dark:bg-gray-900/90 dark:border-indigo-900/50 p-6 lg:p-8">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
          <div>
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
              Kelola data rekening bank dengan mudah
            </p>
          </div>
            ...
            <table class="min-w-max w-full">
              <thead class="bg-gradient-to-r from-indigo-50 to-indigo-100 dark:from-indigo-900/50 dark:to-indigo-800/50">
                <tr>
                  ...
                  <th class="px-6 py-4 text-left text-xs font-bold uppercase text-indigo-700 dark:text-indigo-300">Nama Bank</th>
                  <th class="px-6 py-4 text-left text-xs font-bold uppercase text-indigo-700 dark:text-indigo-300">Nomor Rekening</th>
                  <th class="px-6 py-4 text-left text-xs font-bold uppercase text-indigo-700 dark:text-indigo-300">Atas Nama</th>
              ...
              <tbody>
                <tr
                  ...
                  :key="bank.id"
                  class="hover:bg-indigo-50/50 dark:hover:bg-indigo-900/30 border-b border-gray-200 dark:border-gray-700 last:border-0"
                >
        ...
        <div v-else class="text-center py-16">
          ...
          <p class="text-gray-500 dark:text-gray-400 font-medium">Belum ada data bank</p>
          <p class="text-sm text-gray-400 dark:text-gray-500 mt-1">Tambahkan rekening bank untuk memulai</p>
        </div>
