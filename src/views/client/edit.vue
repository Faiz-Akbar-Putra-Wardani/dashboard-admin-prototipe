...
const fetchClient = async () => {
  try {
    ...
    oldImage.value = client.image ? getImageUrl(client.image) : null;
  } catch (error) {
    Swal.fire({
      icon: "error",
      title: "Gagal!",
      text: "Data klien tidak ditemukan.",
    });
    router.push("/clients");
  }
};
...
const updateClient = async () => {
  ...
  try {
    ...
    Api.defaults.headers.common["Authorization"] = token;
    const response = await Api.put(`/api/clients/${clientId}`, formData);
    ...
    await Swal.fire({
      ...
      text:
        ...
        "Perubahan data klien berhasil disimpan.",
      timer: 2000,
...
// Panggil data kategori saat halaman dibuka
    ...
    >
      <div class="mb-6">
        ...
        <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
          Ubah kolom di bawah untuk memperbarui data kategori.
        </p>
      </div>
      ...
      <form @submit.prevent="updateClient" class="space-y-6">
        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:justify-end">
          ...
          >
            ...
            <span>{{ isSubmitting ? "Menyimpan..." : "Update Klien" }}</span>
          </button>
        </div>
      </form>
    </div>
