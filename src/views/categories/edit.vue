...
const fetchCategory = async () => {
  try {
    ...
    oldImage.value = category.image ? getImageUrl(category.image) : null;
  } catch (error) {
    Swal.fire({
      icon: "error",
      title: "Gagal!",
  ...
  try {
    ...
    await Swal.fire({
      ...
      text:
        ...
        "Perubahan data kategori berhasil disimpan.",
      timer: 2000,
      ...
      <form @submit.prevent="updateCategory" class="space-y-6">
        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:justify-end">
          ...
          >
            ...
            <span>{{ isSubmitting ? "Menyimpan..." : "Perbarui Kategori" }}</span>
          </button>
        </div>
      </form>
    </div>
