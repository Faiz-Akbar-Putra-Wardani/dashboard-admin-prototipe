...
const repairId = route.params.id; // Ambil ID dari route params
...
const fetchRepairData = async () => {
  try {
    ...
    form.start_date = repair.start_date.split('T')[0]; // Format YYYY-MM-DD
    ...
    console.log("Repair data loaded:", repair); // Debug
  } catch (error) {
...
const updateRepair = async () => {
  ...
  try {
    ...
    Api.defaults.headers.common["Authorization"] = token;
    const response = await Api.put(`/api/repairs/${repairId}`, payload);
...
onMounted(async () => {
  await fetchCustomers(); // Fetch customers dulu
  await fetchRepairData(); // Kemudian fetch data repair
   initDatePickers(); // Init date pickers
});
</script>
    ...
    <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] lg:p-6">
      ...
      <div v-if="isLoading" class="flex items-center justify-center py-12">
        <div class="text-center">
          ...
          <p class="text-gray-600 dark:text-gray-400">Memuat data perbaikan...</p>
        </div>
      </div>
      ...
      <div v-else>
        <div class="mb-6">
          <h2 class="text-xl font-semibold text-gray-900 dark:text-white">Edit Perbaikan</h2>
        ...
        <form @submit.prevent="updateRepair" class="space-y-6">
          ...
          <div class="group relative">
            ...
            />
            <p class="mt-1 text-xs text-gray-500">Invoice tidak dapat diubah</p>
          </div>
          ...
          <div class="group relative">
           ...
           <input
            ...
            type="text"
            readonly
            class="block w-full rounded-lg border border-gray-300 bg-gray-50 px-4 py-2.5 text-sm
          ...
          <div class="flex flex-col gap-3 sm:flex-row sm:justify-end">
            ...
            >
              ...
              <span>{{ isSubmitting ? "Memperbarui..." : "Update Perbaikan" }}</span>
            </button>
          </div>
        </form>
      </div>
