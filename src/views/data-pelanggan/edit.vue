...
const currentPageTitle = ref('Edit Data Pelanggan')
...
const getCustomer = async () => {
  try {
    ...
    Swal.fire({
      icon: 'error',
      title: 'Gagal!',
      text: 'Tidak dapat memuat data pelanggan.',
    })
...
const updateCustomer = async () => {
  const confirm = await Swal.fire({
    title: 'Perbarui Data?',
    text: 'Apakah kamu yakin ingin menyimpan perubahan data pelanggan?',
    icon: 'question',
  ...
  try {
    ...
    await Swal.fire({
      icon: 'success',
      title: 'Berhasil!',
      text: response.data.meta?.message || 'Data pelanggan berhasil diperbarui.',
      timer: 2000,
    ...
    <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] lg:p-6">
      <div class="mb-6">
        ...
        <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
          Perbarui informasi pelanggan di bawah ini, lalu simpan perubahan.
        </p>
      </div>

      <!-- Loading State -->
      <div v-if="isLoading" class="text-center py-6 text-gray-600">
        Memuat data pelanggan...
      </div>

      <!-- Form -->
      <form v-else @submit.prevent="updateCustomer" class="space-y-6">
        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:justify-end">
          ...
          >
            ...
            <span>{{ isSubmitting ? 'Menyimpan...' : 'Perbarui Pelanggan' }}</span>
          </button>
        </div>
      </form>
    </div>
